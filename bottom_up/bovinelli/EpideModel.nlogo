...
to setup-people
  ...
  [
    ...
    ifelse n_children > 0 [
      ...
      set relative-recovery-chance recovery-chance * 1.5
      set showing-chance 4
    ] [
      if n_adults > 0 [
        set shape "box"
        set n_adults n_adults - 1
        set relative-recovery-chance recovery-chance
        set showing-chance 3
      ]
    ]

    if n_children <= 0 and n_adults <= 0 [
      set shape "butterfly"
      set recovery-time recovery-time * 2
      set relative-recovery-chance recovery-chance / 1.5
      set showing-chance 2
    ]
...
to go
  ...
  ask turtles with [ not may-symptomatic?]
  [
    maybe-symptom
  ]

  ask turtles with [ may-symptomatic? ]
  [
    symptom
  ]
...
to maybe-recover
  ...
  [
    if random-float 100 < relative-recovery-chance
    [ set infected? false
      set cured? true
...
1329
74
Be careful to mantain percentages coherent\nsum of % should not be > 100
12
0.0
1
...
1323
273
Children are represented by bugs\nAdults are represented by boxes\nOldies are represented by butterflies
12
14.0
1
...
1326
485
Quarantine is a theorethical zone located in the center of the view, no quarantined turtle can infect others.
12
65.0
1
