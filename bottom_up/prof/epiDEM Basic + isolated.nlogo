...
[
  ...
  susceptible?        ;; Tracks whether the person was initially susceptibleù
  isolated?           ;; the inidividuals self-isolated due to symptoms
  ...
  symptom-chance      ;; probability that the turtle has disease symptoms
]
...
to setup-people
  ...
  [
    ...
    set symptom-chance random-normal average-symptom-chance average-symptom-chance / 4
...
to infect  ;; turtle procedure
  let nearby-uninfected 0
  ifelse immunize?
  [set nearby-uninfected (turtles-on neighbors)
  ...
  [set nearby-uninfected (turtles-on neighbors)
     ...
     [ ask nearby-uninfected
       [ if random-float 100 < infection-chance and not isolated? ;; only infect if turtle is not isolated
         [ set infected? true
...
to maybe-isolate
  if random-float 100 < symptom-chance
  [ set isolated? true ]
end
